...
public class MessageConsumer {
    ...
    public void receiveTransaction(StockEntity stockEntity) {
        System.out.println("[MessageConsumer] received message  = " + stockEntity);
        try {
            stockEntity.setProcessed(true);
            stockEntity.setUpdatedAt(LocalDateTime.now());
            stockRepository.save(stockEntity); // 상태 업데이트
            System.out.println("[MessageConsumer] StockEntity 저장 완료 ");
        } catch (Exception e) {
            System.out.println("[MessageConsumer] Entity 수정 에러 " + e.getMessage());
